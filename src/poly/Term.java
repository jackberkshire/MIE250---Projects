 ...
 *  it has two terms 5x^2 and 3xy, each of which would be represented by a different
...
public class Term {
	...
	public double evaluate(Vector assignments) throws PolyException {
                ...
                for (int i = 0; i < _vars.size(); i++) { //check if all vars in var are defined in assignments
                    String s = _vars.get(i);
                    if (assignments.getVal(s) == null)
                        throw new PolyException("variable is not defined");
                }
                
                double result = 1;
                
                for (int i = 0; i < _vars.size(); i++) { //evaulates each term inputing corresponding assignment var pair
                    result = result * Math.pow(assignments.getVal(_vars.get(i)), _pows.get(i));
                }
		return _coef*result; //multiplies and returns the results by the coef
	}
	...
	public Term differentiate(String var) {
            Term partDer = new Term(1);
            
            partDer._coef = this._coef; //sets new term coef to this terms coef
           
            //copy content over to new term
            partDer._vars = new ArrayList<>(this._vars);
            partDer._pows = new ArrayList<>(this._pows);
            
            for (int i = 0; i < this._vars.size(); i++) { //gets the corresponding var to the parameter
                if (partDer._vars.get(i).compareTo(var) == 0) { //checks if the term has the variable we are diff w/ respect to
                    if (partDer._pows.get(i) == 1) { //if the var is to the power of one remove it 
                        partDer._pows.remove(i);
                        partDer._vars.remove(i);
                        return partDer;
                    }
                    else {
                        partDer._coef = partDer._coef * partDer._pows.get(i); //multiplies power to the coefficient
                        partDer._pows.set(i, partDer._pows.get(i) -1); //decreases the power by 1
                        return partDer;
                    }
                }
            }
            for (int i = 0; i <this._vars.size(); i++) { //goes through list if the diff variable isnt in the term
                if (partDer._vars.get(i).equals(var) == false) { //and removes it from the list and returns null
                    partDer._pows.remove(i);
                    partDer._vars.remove(i);
                    partDer._coef = 0;
                    return null;
            ...
            if (partDer._vars.size() == 0) { //if term is constant return null
                partDer._coef = 0;
                return null;
            }
            return partDer;
        ...
        */
        public static void main(String[] args) throws PolyException {
            Term term1 = new Term("2*x^2*y*z^2");
            System.out.println(term1); //this is term 1
            System.out.println(term1.differentiate("x")); //term 1 differentiated with x
            System.out.println(term1.differentiate("y")); //with y
            System.out.println(term1.differentiate("z")); //with z
            System.out.println(term1); //term1 should be unchanged
            Vector vec3 = new Vector("{ x=-1 y=-2.0 z=3d w=2 }");
            System.out.println(term1.evaluate(vec3));
