...
public class Minimizer {
	...
	public void minimize(Polynomial p) throws Exception {
		...
		// TODO: Build the partial derivatives of p for all variables in p
                //^^^see function computeGradient in Polynomial
                _var2gradp = p.computeGradient();
		...
		//	}
                while (_nIter < _maxIter && _lastGradNorm > _eps) {
                    _nIter++; //increment iterations
                    //compute the normal of the gradient that has been evaluated (is now a vector)
                    _lastGradNorm = Polynomial.computeGradientNorm(Polynomial.computeGradientEval(_var2gradp, _lastx)); 
                    //sets the 
                    _lastx = _lastx.sum(Polynomial.computeGradientEval(_var2gradp, _lastx).scalarMult(-_stepSize));
