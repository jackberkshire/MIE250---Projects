...
public class Vector {
	...
	public Vector(String s) {
            _hmVar2Value = new HashMap<String,Double>();
            String[] keyValuePairs = s.split(" "); //splits string around the spaces
            
            for (int i = 1; i < keyValuePairs.length-1; i++) { //iterates through each split string
                String pair = keyValuePairs[i];
                String[] keyValue = pair.split("="); //splits the split string at the equals
                _hmVar2Value.put(keyValue[0], Double.parseDouble(keyValue[1])); //adds the values to the hashmap
            }          
	}
	...
	public void setAll(Vector x) {
        ...
        public String toString() {
            ...
            for (HashMap.Entry<String, Double> pair : _hmVar2Value.entrySet()) {
                sb.append(String.format("%s=%6.4f ", pair.getKey(), pair.getValue())); 
            }
            sb.append("}");
            return sb.toString();
        }
        /**
         * adds two vectors toegther
         */
        public Vector sum(Vector v) {
            Vector vSum = new Vector();
            
            for (HashMap.Entry<String, Double> pair : _hmVar2Value.entrySet()) { //goes through all pairs of var, values
                String s = pair.getKey();
                vSum.set(s, _hmVar2Value.get(s) + v._hmVar2Value.get(s)); //sums this vector with 
            }
            return vSum;
        }
        
        /**
         * multiplied the vector by a scalar
         */
        public Vector scalarMult(double d) {
            Vector vScalarMult = new Vector();
            
            for (HashMap.Entry<String, Double> pair : _hmVar2Value.entrySet()) { //goes through all pairs of var
                String s = pair.getKey();
                vScalarMult.set(s, _hmVar2Value.get(s) * d); //multiplies this vector by scalar mult
            }
            return vScalarMult;
        }
        
        /** computer norm of the vector
         * 
         */
        public Double computeL2Norm() {
            double sum = 0;
            
            for (HashMap.Entry<String, Double> pair : _hmVar2Value.entrySet()) { //goes through all pairs
                String s = pair.getKey();
                sum = sum + _hmVar2Value.get(s) * _hmVar2Value.get(s); //adds the squares of all values
            }
        ...
        public boolean equals(Object o) {
			...
			Vector v = (Vector)o;
                        ...
                        for (HashMap.Entry<String, Double> pair : _hmVar2Value.entrySet()) { //check has all same values for keys
                            String s = pair.getKey();
                            if (v._hmVar2Value.containsKey(s) == false)
                                return false;
                        }
                        for (HashMap.Entry<String, Double> pair : _hmVar2Value.entrySet()) { //check has all same values for keys
                            String s = pair.getKey();
                            if (v._hmVar2Value.containsValue(_hmVar2Value.get(s)) == false)
                                return false;
                        }
                      return true;  
            }
        ...
        /**
         *  gets the value of the vector at a given variable
         */
        ...
        /*
        * returns all the keys in the vector
        */
        public TreeSet<String> getAllVars() {
            return new TreeSet<String>(_hmVar2Value.keySet());
        }
